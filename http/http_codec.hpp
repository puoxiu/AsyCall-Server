...
有：
    ...
    m_body："This is a body."
    m_headline："GET /index.html HTTP/1.1"
...
struct _http_base_parser {
    ...
    */
    std::string _headline_second() {
        ...
        auto res = line.substr(space1 + 1, space2 - space1 - 1);
        // fmt::print("url: {}\n", res);
        return res;
    }
    /*
        * @brief 解析请求头部的第三个字段--HTTP 版本
        * @return HTTP/1.1
    */
...
struct http_request_parser : _http_base_parser<HeaderParser> {
    HttpMethod method() {
        // 返回请求方法的字符串
        std::string m = this->_headline_first();
